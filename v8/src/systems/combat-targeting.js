...
function priorityTargetScore(unit, target, distance) {
  ...
  const meleeActor = !!(unit && (unit.arch === 'tank' || unit.arch === 'melee' || unit.prefersMelee || unit.paladinHybrid));
  if (preferred === family) score -= 260;
  else if (preferred === 'melee' && meleeActor) score -= 280;
...
export function effectiveArenaAttackRange(actor, bounds = {}) {
  const raw = actor && Number.isFinite(actor.range) ? actor.range : 40;
  if (!actor || !(bounds.arenaPhase || bounds.inArena)) return raw;
  if (actor.isBoss || actor.isElite || actor.isBarrier || actor.isAerial) return raw;
  if (isArenaRangedActor(actor)) {
    if (actor.isEnemy) return Math.min(raw, 165);
    if (actor.arch === 'healer') return Math.min(raw, 165);
    return Math.min(raw, 175);
  }
  ...
  if (actor.isPlayer && actor.homeX != null) {
    ...
    if (yOk && xOk) {
      ...
      if (curOutsideY) {
        ...
        const curForwardOver = curDyHsigned < -bounds.leashForward ? (-bounds.leashForward - curDyHsigned) : 0;
        const newForwardOver = newDyHsigned < -bounds.leashForward ? (-bounds.leashForward - newDyHsigned) : 0;
        const curOver = curForwardOver || (curDyHsigned - bounds.leashBack);
        const newOver = newForwardOver || (newDyHsigned > bounds.leashBack ? (newDyHsigned - bounds.leashBack) : 0);
...
export function isReachableFromLeash(unit, target, bounds) {
  ...
  if (target.winterWhelp && target._dragonBoss && target._dragonBoss._dragonSkyPhase) return true;
