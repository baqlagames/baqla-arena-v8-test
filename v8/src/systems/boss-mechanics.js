...
function updateStormWardOvercharge(ward,b,ctx){
  ...
  const text=second?'DEEP FREEZE':'CRYSTAL OVERCHARGE';
  groundFx.push({x:ward.x,y:ward.y,r:0,maxR:Math.max(82,ward.size*3.4),life:0.58,color,enemyWarn:true,warnTimer:24,warnMax:24,label:second?'DEEP':'CRYSTAL'});
...
function initStormboundVizier(b,ctx){
  ...
  if(b.winterglassMagistrate&&!b._winterglassArrivalDone){
    b._winterglassArrivalDone=true;
    b._winterglassArrivalTimer=120;
    const groundFx=ctx.groundFx||[];
    const addP=ctx.addParticle||(()=>{});
    groundFx.push({x:b.x,y:b.y,r:0,maxR:Math.max(190,(b.size||42)*3.9),life:0.95,color:'#d8f8ff',celestialAuraFx:true,label:'FROST'});
    groundFx.push({x:b.x,y:b.y,r:0,maxR:Math.max(130,(b.size||42)*2.6),life:0.7,color:'#9fdcff',enemyWarn:true,warnTimer:32,warnMax:32,label:'WINTER'});
    for(let i=0;i<34;i++)addP(b.x+rnd(-(b.size||42)*1.2,(b.size||42)*1.2),b.y+rnd(-(b.size||42)*0.9,(b.size||42)*0.8),i%2?'#d8f8ff':'#9fdcff',1,4);
    if(ctx.showFlash)ctx.showFlash('WINTERGLASS ARRIVAL!','#d8f8ff',55);
  }
...
function stormExposeVizier(b,ctx){
  ...
  addDmg(b.x,b.y-(b.size||40)-16,'MAGISTRATE EXPOSED','#d8f8ff',{sz:13,bold:true,outline:'#061433'});
  groundFx.push({x:b.x,y:b.y,r:0,maxR:Math.max(160,(b.size||42)*3.2),life:0.65,color:'#d8f8ff',celestialAuraFx:true});
  for(let i=0;i<28;i++)addP(b.x+rnd(-b.size,b.size),b.y+rnd(-b.size,b.size),'#d8f8ff',1,4);
  showFlash('MAGISTRATE EXPOSED!','#d8f8ff',55);
  shake(5);
}
function applyStormVenom(b,ctx){
  ...
  groundFx.push({x:b.x,y:b.y,r:0,maxR:Math.max(170,(b.size||42)*3.4),life:0.55,color:'#d8f8ff',celestialAuraFx:true,label:'FROSTBURN'});
  for(const u of targets){
    ...
    u._stormVenomFrom=b;
    beamFx.push({x1:b.x,y1:b.y-(b.size||40)*0.35,x2:u.x,y2:u.y-u.size*0.25,life:20,maxLife:20,color:'#d8f8ff',width:3.5,straight:false});
    groundFx.push({x:u.x,y:u.y,r:0,maxR:Math.max(36,(u.size||20)*2.0),life:0.42,color:'#d8f8ff',enemyWarn:true,warnTimer:18,warnMax:18,label:'FROSTBURN'});
    addDmg(u.x,u.y-(u.size||20)-12,'FROSTBURN','#d8f8ff',{sz:10,bold:true,outline:'#061433'});
...
function resolveStormWards(b,ctx){
  ...
  groundFx.push({x:b.x,y:b.y,r:0,maxR:Math.max(150,(b.size||42)*3.1),life:0.58,color:'#d8f8ff',celestialAuraFx:true});
  addDmg(b.x,b.y-(b.size||40)-30,'WINTERGLASS BARRIER BROKEN','#d8f8ff',{sz:11,bold:true,outline:'#061433'});
  for(let i=0;i<24;i++)addP(b.x+rnd(-b.size,b.size),b.y+rnd(-b.size,b.size),'#d8f8ff',1,4);
  showFlash('WINTERGLASS BARRIER BROKEN!','#d8f8ff',45);
  shake(4);
...
function castStormTwinWards(b,ctx,thresholdIndex=0){
  ...
  const defs=[
    {kind:'iron',name:'Frostglass Prism',x:-96,color:'#9fdcff',accent:'#eef8ff',armor:16,magicRes:1,armorType:'heavy',preferredBy:'magic',label:'MAGIC',first:b.ironSurgeFirst||90,every:b.ironSurgeEvery||180},
    {kind:'mirror',name:'Mirrorice Bulwark',x:96,color:'#d8f8ff',accent:'#8fb8d8',armor:1,magicRes:18,armorType:'warded',preferredBy:'physical',label:'PHYSICAL',first:b.mirrorCleaveFirst||72,every:b.mirrorCleaveEvery||144},
  ];
  const refs=[];
  for(const def of defs){
    const ward={
      ...
      armor:def.armor,magicRes:def.magicRes,armorType:def.armorType,projType:'frost',
  ...
  addDmg(b.x,b.y-(b.size||40)-10,'WINTERGLASS CRYSTALS','#d8f8ff',{sz:13,bold:true,outline:'#061433'});
  addDmg(b.x,b.y-(b.size||40)-26,'WINTERGLASS BARRIER','#eef8ff',{sz:11,bold:true,outline:'#061433'});
  groundFx.push({x:b.x,y:b.y,r:0,maxR:Math.max(150,(b.size||42)*3.0),life:0.72,color:'#d8f8ff',celestialAuraFx:true,label:'BARRIER'});
  showFlash('WINTERGLASS CRYSTALS!','#d8f8ff',55);
  shake(5);
  syncStormVizierCooldowns(b,ctx);
  return true;
}
function castStormIronSurge(ward,b,ctx){
  ...
  groundFx.push({x:ward.x,y:ward.y,r:0,maxR:210,life:0.52,color:'#d8f8ff',celestialAuraFx:true,label:'RIME'});
  for(const u of targets){
    beamFx.push({x1:ward.x,y1:ward.y-ward.size*0.3,x2:u.x,y2:u.y-u.size*0.2,life:20,maxLife:20,color:'#d8f8ff',width:4,straight:false});
    dealDamage(u,dmg,ward,'magic','rimeSurge',{sourceLabel:'RIME SURGE',sourceColor:'#d8f8ff'});
    addDmg(u.x,u.y-(u.size||20)-8,'RIME SURGE','#d8f8ff',{sz:10,bold:true,outline:'#061433'});
    addP(u.x,u.y,'#d8f8ff',7,3);
  }
  addDmg(ward.x,ward.y-ward.size-8,'RIME SURGE','#d8f8ff',{sz:12,bold:true,outline:'#061433'});
...
function castStormMirrorCleave(ward,b,ctx){
  ...
  groundFx.push({x:ward.x,y:ward.y,r:0,maxR:radius,life:0.46,color:'#9fdcff',enemyWarn:true,warnTimer:22,warnMax:22,label:'GLACIAL'});
  for(const u of targets){
    const mult=stormIsTank(u)?1:0.84;
    beamFx.push({x1:ward.x,y1:ward.y-ward.size*0.15,x2:u.x,y2:u.y-u.size*0.05,life:18,maxLife:18,color:'#9fdcff',width:5,straight:true});
    dealDamage(u,Math.round(dmg*mult),ward,'normal','glacialCleave',{sourceLabel:'GLACIAL CLEAVE',sourceColor:'#9fdcff'});
    addDmg(u.x,u.y-(u.size||20)-8,'GLACIAL CLEAVE','#9fdcff',{sz:10,bold:true,outline:'#061433'});
    addP(u.x,u.y,'#9fdcff',8,3);
  }
  addDmg(ward.x,ward.y-ward.size-8,'GLACIAL CLEAVE','#9fdcff',{sz:12,bold:true,outline:'#061433'});
...
function castStormSilencingDecree(b,ctx){
  ...
  beamFx.push({x1:b.x,y1:b.y-(b.size||40)*0.45,x2:target.x,y2:target.y-target.size*0.35,life:22,maxLife:22,color:'#d8f8ff',width:5,straight:false});
  ...
  groundFx.push({x:target.x,y:target.y,r:0,maxR:58,life:0.44,color:'#d8f8ff',enemyWarn:true,warnTimer:18,warnMax:18,label:'VOICE'});
  addDmg(target.x,target.y-(target.size||20)-10,'FROZEN VOICE','#d8f8ff',{sz:12,bold:true,outline:'#061433'});
  addDmg(b.x,b.y-(b.size||40)-12,'FROZEN VOICE','#d8f8ff',{sz:13,bold:true,outline:'#061433'});
...
function castStormTankCurse(b,ctx){
  ...
  dealDamage(target,dmg,b,'magic','rimeCurse',{sourceLabel:'RIME CURSE',sourceColor:'#9fdcff'});
  ...
  beamFx.push({x1:b.x,y1:b.y-(b.size||40)*0.35,x2:target.x,y2:target.y-target.size*0.2,life:20,maxLife:20,color:'#9fdcff',width:5,straight:false});
  groundFx.push({x:target.x,y:target.y,r:0,maxR:62,life:0.5,color:'#9fdcff',enemyWarn:true,warnTimer:20,warnMax:20,label:'CURSE'});
  addDmg(target.x,target.y-(target.size||20)-10,'RIME CURSE','#9fdcff',{sz:12,bold:true,outline:'#061433'});
  addDmg(b.x,b.y-(b.size||40)-12,'RIME CURSE','#9fdcff',{sz:13,bold:true,outline:'#061433'});
  addP(target.x,target.y,'#9fdcff',14,3);
  showFlash('RIME CURSE!','#9fdcff',42);
...
function castStormChainDecree(b,ctx){
  ...
  targets.forEach((t,i)=>{
    ...
    beamFx.push({x1:from.x,y1:from.y,x2:t.x,y2:t.y,life:18,maxLife:18,color:i%2?'#eef8ff':'#9fdcff',width:4,straight:false});
    dealDamage(t,Math.round((b.chainDecreeDmg||76)*mult*stormSkillDamageMult(b)),b,'magic','iceChain',{sourceLabel:'ICE CHAIN',sourceColor:'#9fdcff'});
    addDmg(t.x,t.y-(t.size||20)-8,'ICE CHAIN','#9fdcff',{sz:11,bold:true,outline:'#061433'});
    addP(t.x,t.y,'#9fdcff',8,3);
    from=t;
  });
  groundFx.push({x:b.x,y:b.y,r:0,maxR:150,life:0.45,color:'#d8f8ff',flatten:true});
  b._stormChainCasts=(b._stormChainCasts||0)+1;
  addDmg(b.x,b.y-(b.size||40)-10,'ICE CHAIN','#9fdcff',{sz:13,bold:true,outline:'#061433'});
  showFlash('ICE CHAIN!','#9fdcff',45);
  shake(5);
  return true;
}
function castStormGroundingPulse(b,ctx){
  ...
  groundFx.push({x:b.x,y:b.y,r:0,maxR:radius,life:0.62,color:'#d8f8ff',enemyWarn:true,warnTimer:26,warnMax:26,label:'RING'});
  if(shockTargets.length)groundFx.push({x:b.x,y:b.y,r:0,maxR:shockRadius,life:0.46,color:'#9fdcff',enemyWarn:true,warnTimer:22,warnMax:22,label:'SHOCK'});
  for(const u of frontTargets){
    ...
    dealDamage(u,Math.round((b.groundingPulseDmg||88)*mult*skillMult),b,'magic','permafrost',{sourceLabel:'PERMAFROST',sourceColor:'#d8f8ff'});
    addDmg(u.x,u.y-(u.size||20)-8,'PERMAFROST','#d8f8ff',{sz:11,bold:true,outline:'#061433'});
    addP(u.x,u.y,'#d8f8ff',7,3);
    if(tankish||melee){
      ...
      addDmg(u.x,u.y-(u.size||20)-20,'FROSTBITE BRAND','#9fdcff',{sz:10,bold:true,outline:'#061433'});
    }
  }
  for(const u of shockTargets){
    const dmg=Math.round((b.groundingPulseDmg||88)*(b.groundingStormShockMult||0.42)*skillMult);
    dealDamage(u,dmg,b,'magic','frostShock',{sourceLabel:'FROST SHOCK',sourceColor:'#9fdcff'});
    addDmg(u.x,u.y-(u.size||20)-8,'FROST SHOCK','#9fdcff',{sz:11,bold:true,outline:'#061433'});
    addP(u.x,u.y,'#9fdcff',6,3);
  }
  b._stormGroundingCasts=(b._stormGroundingCasts||0)+1;
  addDmg(b.x,b.y-(b.size||40)-10,'PERMAFROST RING','#d8f8ff',{sz:13,bold:true,outline:'#061433'});
  showFlash('PERMAFROST RING!','#d8f8ff',42);
  shake(5);
  return true;
}
function castStormCourtPulse(b,ctx){
  ...
  groundFx.push({x:b.x,y:b.y,r:0,maxR:radius,life:0.58,color:'#d8f8ff',enemyWarn:true,warnTimer:24,warnMax:24,label:'WHITEOUT'});
  groundFx.push({x:b.x,y:b.y,r:0,maxR:Math.max(88,(b.size||42)*1.9),life:0.5,color:'#9fdcff',celestialAuraFx:true});
  for(const u of targets){
    const tankish=stormIsTank(u);
    const melee=stormIsMelee(u)&&!tankish;
    ...
    beamFx.push({x1:b.x,y1:b.y-(b.size||40)*0.55,x2:u.x,y2:u.y-u.size*0.25,life:20,maxLife:20,color:tankish?'#eef8ff':'#9fdcff',width:tankish?5:3.5,straight:false});
    dealDamage(u,dmg,b,'magic','whiteoutPulse',{sourceLabel:'WHITEOUT',sourceColor:'#d8f8ff'});
    addDmg(u.x,u.y-(u.size||20)-8,'WHITEOUT','#d8f8ff',{sz:11,bold:true,outline:'#061433'});
    addP(u.x,u.y,tankish?'#eef8ff':'#9fdcff',7,3);
  }
  b._stormCourtPulseCasts=(b._stormCourtPulseCasts||0)+1;
  addDmg(b.x,b.y-(b.size||40)-10,'WHITEOUT PULSE','#d8f8ff',{sz:13,bold:true,outline:'#061433'});
  showFlash('WHITEOUT PULSE!','#d8f8ff',45);
  shake(5);
  return true;
}
...
function dragonStormThresholds(b){
  const authored=Array.isArray(b&&b.diamondStormThresholds)?b.diamondStormThresholds:[b&&b.diamondStormAt];
  const thresholds=authored.filter(value=>Number.isFinite(value)&&value>0&&value<1).sort((a,bb)=>bb-a);
  return thresholds.length?thresholds:[0.5];
}
function dragonNextStormThreshold(b){
  const thresholds=b._dragonStormThresholds||dragonStormThresholds(b);
  const idx=Math.max(0,Math.floor(b._dragonSkyThresholdIndex||0));
  return idx<thresholds.length?thresholds[idx]:null;
}
function dragonAngleDiff(a,b){
  let d=a-b;
  while(d>Math.PI)d-=Math.PI*2;
  while(d<-Math.PI)d+=Math.PI*2;
  return Math.abs(d);
}
function dragonLineDistance(px,py,x1,y1,x2,y2){
...
function dragonPathPoint(points,pct){
  const segs=[];
  let total=0;
  ...
  let walk=Math.max(0,Math.min(1,pct))*Math.max(1,total);
  for(const seg of segs){
    if(walk>seg.len){walk-=seg.len;continue}
    const t=seg.len>0?walk/seg.len:0;
    return {
  ...
  return last?{x:last.b.x,y:last.b.y,dx:last.b.x-last.a.x,dy:last.b.y-last.a.y}:points[0];
}
function applyDragonRimeVenom(u,b,ctx){
  if(!u||u.hp<=0)return;
  u._rimeVenomTimer=Math.max(u._rimeVenomTimer||0,b.rimeVenomDur||5*GAME_TICK_HZ);
  u._rimeVenomTick=Math.min(u._rimeVenomTick||GAME_TICK_HZ,GAME_TICK_HZ);
  u._rimeVenomHpPct=b.rimeVenomHpPct||0.008;
  u._rimeVenomMinDmg=b.rimeVenomMinDmg||8;
  u._rimeVenomHealCut=b.rimeVenomHealCut||0.08;
  u._rimeVenomFrom=b;
  ctx.addDamageText(u.x,u.y-(u.size||20)-20,'RIME VENOM','#bff4ff',{sz:10,bold:true,outline:'#061433'});
  ctx.addParticle(u.x,u.y-u.size*0.45,'#bff4ff',5,2);
}
function syncDragonCooldowns(b){
  ...
  cd.frigidMaw=b._dragonSkyPhase?hidden:Math.max(0,Math.round(b._dragonMawCd||0));
  cd.iceWall=b._dragonSkyPhase?hidden:Math.max(0,Math.round(b._dragonWallCd||0));
  cd.diamondStorm=nextStorm==null?hidden:(b._dragonSkyPhase?0:Math.max(0,Math.round(b._dragonStormCd||0)));
...
function initWinterglassDragon(b,ctx){
  ...
  b._dragonMawCd=b.frigidMawFirst||600;
  b._dragonWallCd=b.iceWallFirst||1080;
  b._dragonStormCd=b.diamondStormCD||480;
  b._dragonPendingComets=[];
  b._dragonIceWalls=[];
  b._dragonStormThresholds=dragonStormThresholds(b);
  b._dragonSkyThresholdIndex=0;
...
function tickDragonCooldowns(b){
  ...
  if(b._dragonMawCd>0)b._dragonMawCd--;
  if(b._dragonWallCd>0)b._dragonWallCd--;
...
function castDragonIceComets(b,ctx){
  ...
  const fall=b.iceCometFall||54;
  const radius=b.iceCometRadius||58;
  b._dragonPendingComets=Array.isArray(b._dragonPendingComets)?b._dragonPendingComets:[];
  targets.forEach((u,i)=>{
    const x=u.x,y=u.y;
    b._dragonPendingComets.push({target:u,x,y,t:warn+i*7,fall,radius,dmg:Math.round((b.iceCometDmg||138)*dragonSkillDamageMult(b))});
    groundFx.push({x,y,r:0,maxR:radius,life:1.0,color:'#9fdcff',enemyWarn:true,warnTimer:warn,warnMax:warn,label:'COMET'});
    beamFx.push({x1:x+rnd(-12,12),y1:ctx.arenaTop+18,x2:x,y2:y,life:warn,maxLife:warn,color:'#d8f8ff',width:2.2,straight:true});
...
function tickDragonComets(b,ctx){
  ...
  for(let i=pending.length-1;i>=0;i--){
    ...
    if(c.t>0){
      const fall=Math.max(1,c.fall||b.iceCometFall||54);
      if(c.t<=fall&&(c.t%3===0||c.t<=3)){
        groundFx.push({x,y,r:0,maxR:c.radius,life:0.20,color:'#d8f8ff',iceCometFall:true,progress:1-(c.t/fall),height:170});
      }
      continue;
    }
    groundFx.push({x,y,r:0,maxR:c.radius,life:0.55,color:'#d8f8ff',enemyWarn:true,warnTimer:18,warnMax:18,label:'COMET'});
    beamFx.push({x1:x+rnd(-16,16),y1:ctx.arenaTop+8,x2:x,y2:y,life:20,maxLife:20,color:'#eef8ff',width:6.4,straight:true});
    const hit=new Set();
...
function castDragonFrigidMaw(b,ctx){
  const { units, groundFx, beamFx, addParticle:addP, addDamageText:addDmg, showFlash, shake }=ctx;
  const players=dragonPlayers(units);
  if(!players.length)return false;
  const focus=players.find(dragonIsTank)||players.sort((a,bb)=>((a.hp/a.maxHp)-(bb.hp/bb.maxHp)))[0];
  if(!focus)return false;
  const angle=Math.atan2(focus.y-b.y,focus.x-b.x);
  const range=b.frigidMawRange||260;
  const arc=b.frigidMawArc||0.86;
  const base=(b.frigidMawDmg||126)*dragonSkillDamageMult(b);
  const warn=48;
  b._dragonPendingMaws=Array.isArray(b._dragonPendingMaws)?b._dragonPendingMaws:[];
  b._dragonPendingMaws.push({angle,range,arc,dmg:Math.round(base),t:warn,focus});
  groundFx.push({x:b.x,y:b.y,r:0,maxR:range,life:0.9,color:'#bff4ff',enemyWarn:true,warnTimer:warn,warnMax:warn,warnKind:'cleave',warnAngle:angle,warnSpread:arc,label:'MAW'});
  for(let i=-2;i<=2;i++){
    const a=angle+i*arc*0.22;
    beamFx.push({x1:b.x,y1:b.y-b.size*0.18,x2:b.x+Math.cos(a)*range,y2:b.y+Math.sin(a)*range,life:warn,maxLife:warn,color:'#d8f8ff',width:i===0?4.8:2.2,straight:true});
  }
  addDmg(b.x,b.y-(b.size||70)-14,'FRIGID MAW','#bff4ff',{sz:13,bold:true,outline:'#061433'});
  showFlash('FRIGID MAW!','#bff4ff',45);
  b._dragonMawCasts=(b._dragonMawCasts||0)+1;
  shake(6);
  return true;
}
function tickDragonFrigidMaw(b,ctx){
  const pending=Array.isArray(b._dragonPendingMaws)?b._dragonPendingMaws:[];
  if(!pending.length)return;
  const { units, groundFx, beamFx, dealDamage, addParticle:addP, addDamageText:addDmg, shake }=ctx;
  for(let i=pending.length-1;i>=0;i--){
    const maw=pending[i];
    maw.t--;
    if(maw.t>0){
      if(maw.t%8===0)groundFx.push({x:b.x,y:b.y,r:0,maxR:maw.range,life:0.24,color:'#bff4ff',enemyWarn:true,warnTimer:8,warnMax:8,warnKind:'cleave',warnAngle:maw.angle,warnSpread:maw.arc,label:'MAW'});
      continue;
    }
    let hits=0;
    for(let ray=-2;ray<=2;ray++){
      const a=maw.angle+ray*maw.arc*0.18;
      beamFx.push({x1:b.x,y1:b.y-b.size*0.20,x2:b.x+Math.cos(a)*maw.range,y2:b.y+Math.sin(a)*maw.range,life:22,maxLife:22,color:'#eef8ff',width:ray===0?8:4.2,straight:true});
    }
    for(const u of dragonPlayers(units)){
      const dx=u.x-b.x,dy=u.y-b.y,d=Math.hypot(dx,dy);
      if(d>maw.range+(u.size||20)*0.45)continue;
      if(dragonAngleDiff(Math.atan2(dy,dx),maw.angle)>maw.arc*0.52)continue;
      const mult=dragonIsTank(u)?(b.frigidMawTankMult||1.05):(dragonIsMelee(u)?(b.frigidMawMeleeMult||0.95):(b.frigidMawBacklineMult||0.65));
      dealDamage(u,Math.round((maw.dmg||126)*mult),b,'magic','frigidMaw',{sourceLabel:'FRIGID MAW',sourceColor:'#bff4ff'});
      applyDragonRimeVenom(u,b,ctx);
      addDmg(u.x,u.y-(u.size||20)-8,'FRIGID MAW','#bff4ff',{sz:11,bold:true,outline:'#061433'});
      addP(u.x,u.y,'#d8f8ff',8,3);
      hits++;
    }
    if(!hits&&maw.focus&&maw.focus.hp>0){
      dealDamage(maw.focus,Math.round((maw.dmg||126)*(dragonIsTank(maw.focus)?0.90:0.62)),b,'magic','frigidMaw',{sourceLabel:'FRIGID MAW',sourceColor:'#bff4ff'});
      applyDragonRimeVenom(maw.focus,b,ctx);
    }
    shake(6);
    pending.splice(i,1);
  }
}
function castDragonIceWall(b,ctx){
  const { units, groundFx, beamFx, addParticle:addP, addDamageText:addDmg, showFlash, shake }=ctx;
  const players=dragonPlayers(units);
  if(!players.length)return false;
  const target=players.sort((a,bb)=>((a.hp/a.maxHp)-(bb.hp/bb.maxHp)))[0];
  const leftStart=(target.x||b.x)>((ARENA_L+ARENA_R)/2);
  const yBase=target.y||b.y+160;
  const start=clampBossPoint(leftStart?ARENA_L+24:ARENA_R-24,yBase-150,ctx,{sideMargin:18,topMargin:86,bottomMargin:66});
  const mid=clampBossPoint(b.x+(leftStart?74:-74),yBase+rnd(-36,36),ctx,{sideMargin:38,topMargin:90,bottomMargin:66});
  const end=clampBossPoint(leftStart?ARENA_R-24:ARENA_L+24,yBase+140,ctx,{sideMargin:18,topMargin:86,bottomMargin:66});
  const warn=b.iceWallWarn||42;
  const wall={points:[start,mid,end],warn,t:0,dur:b.iceWallDur||150,width:b.iceWallWidth||30,length:b.iceWallLength||145,dmg:Math.round((b.iceWallDmg||96)*dragonSkillDamageMult(b)),hit:new Set()};
  b._dragonIceWalls=Array.isArray(b._dragonIceWalls)?b._dragonIceWalls:[];
  b._dragonIceWalls.push(wall);
  for(let i=0;i<wall.points.length-1;i++){
    const a=wall.points[i],bb=wall.points[i+1];
    groundFx.push({x:a.x,y:a.y,x2:bb.x,y2:bb.y,r:0,maxR:32,life:0.78,color:'#d8f8ff',enemyWarn:true,warnTimer:warn,warnMax:warn,warnKind:'line',width:wall.width,label:'WALL'});
    beamFx.push({x1:a.x,y1:a.y,x2:bb.x,y2:bb.y,life:warn,maxLife:warn,color:'#9fdcff',width:2.2,straight:true});
  }
  addDmg(b.x,b.y-(b.size||70)-14,'ICEBREAKER WALL','#d8f8ff',{sz:13,bold:true,outline:'#061433'});
  showFlash('ICEBREAKER WALL!','#d8f8ff',45);
  for(let i=0;i<18;i++)addP(mid.x+rnd(-50,50),mid.y+rnd(-25,25),'#d8f8ff',1,3);
  b._dragonIceWallCasts=(b._dragonIceWallCasts||0)+1;
  shake(5);
  return true;
}
function tickDragonIceWalls(b,ctx){
  const walls=Array.isArray(b._dragonIceWalls)?b._dragonIceWalls:[];
  if(!walls.length)return;
  const { units, groundFx, beamFx, dealDamage, addParticle:addP, addDamageText:addDmg, shake }=ctx;
  for(let i=walls.length-1;i>=0;i--){
    const wall=walls[i];
    if(wall.warn>0){wall.warn--;continue}
    wall.t++;
    const pct=wall.t/Math.max(1,wall.dur||150);
    const p=dragonPathPoint(wall.points,pct);
    const d=Math.hypot(p.dx||1,p.dy||0)||1;
    const nx=-(p.dy||0)/d,ny=(p.dx||1)/d;
    const half=(wall.length||145)*0.5;
    const x1=p.x-nx*half,y1=p.y-ny*half,x2=p.x+nx*half,y2=p.y+ny*half;
    beamFx.push({x1,y1,x2,y2,life:12,maxLife:12,color:'#eef8ff',width:8,straight:true});
    beamFx.push({x1:p.x-(p.dx/d)*26,y1:p.y-(p.dy/d)*26,x2:p.x+(p.dx/d)*26,y2:p.y+(p.dy/d)*26,life:10,maxLife:10,color:'#9fdcff',width:13,straight:true});
    if(wall.t%8===0)groundFx.push({x:p.x,y:p.y,r:0,maxR:Math.max(30,wall.width+10),life:0.28,color:'#d8f8ff',enemyWarn:true,warnTimer:8,warnMax:8,label:'WALL'});
    for(const u of dragonPlayers(units)){
      if(wall.hit.has(u))continue;
      const hit=dragonLineDistance(u.x,u.y,x1,y1,x2,y2);
      if(hit.dist>(wall.width||30))continue;
      const mult=dragonIsTank(u)?0.95:(dragonIsMelee(u)?0.90:0.72);
      dealDamage(u,Math.round((wall.dmg||96)*mult),b,'magic','iceWall',{sourceLabel:'ICE WALL',sourceColor:'#d8f8ff'});
      applyDragonRimeVenom(u,b,ctx);
      addDmg(u.x,u.y-(u.size||20)-8,'ICE WALL','#d8f8ff',{sz:11,bold:true,outline:'#061433'});
      addP(u.x,u.y,'#d8f8ff',10,3);
      wall.hit.add(u);
      shake(4);
    }
    if(wall.t>=wall.dur)walls.splice(i,1);
  }
}
...
function anchorWinterglassDragon(b,ctx,sky=false){
  ...
  const liveFrontline=players.some(u=>dragonIsTank(u)||dragonIsMelee(u));
...
function dragonJudgmentPhaseIndex(b){
  ...
  return Math.max(0,Math.min(scales.length-1,((b._dragonSkyThresholdIndex||1)-1)));
}
function dragonJudgmentGuardScale(b){
  const scales=Array.isArray(b.dragonGuardScales)?b.dragonGuardScales:[1,1.12,1.25];
...
function spawnDragonSkyGuard(b,ctx,kind,slotX,scale){
  ...
  const name=magic?'Frostglass Colossus':'Mirrorice Juggernaut';
  ...
  enemies.push(guard);
  groundFx.push({x:guard.x,y:guard.y,r:0,maxR:Math.max(78,guard.size+34),life:0.76,color:guard.color,enemyWarn:true,warnTimer:30,warnMax:30,label:magic?'MAGIC':'PHYSICAL'});
  addDmg(guard.x,guard.y-guard.size-16,name.toUpperCase(),guard.color,{sz:12,bold:true,outline:'#061433'});
  addP(guard.x,guard.y,'#d8f8ff',24,4);
  return guard;
}
function spawnDragonJudgmentGuards(b,ctx){
  if(b._dragonMainPackSpawned)return;
  b._dragonMainPackSpawned=true;
  const scale=dragonJudgmentGuardScale(b);
  const gap=Math.max(78,(b.size||70)*1.05);
  spawnDragonSkyGuard(b,ctx,'magic',-gap,scale);
  spawnDragonSkyGuard(b,ctx,'physical',gap,scale);
  b._dragonGuardSpawned=2;
  ctx.addDamageText(b.x,b.y+(b.size||70)+92,'JUDGMENT GUARDS','#d8f8ff',{sz:13,bold:true,outline:'#061433'});
}
function allDragonSkyGuards(b,enemies){
  return (enemies||[]).filter(enemy=>enemy&&enemy._dragonBoss===b&&enemy.dragonSkyGuard);
}
function activeDragonSkyAdds(b,enemies){
  return allDragonSkyGuards(b,enemies).filter(enemy=>enemy.hp>0);
}
function tickDragonSkyGuard(guard,b,ctx){
  ...
  if(guard.dragonGuardKind==='magic'){
    ...
    for(const u of players){
      ...
      addP(u.x,u.y,'#d8f8ff',5,2.5);
    }
    addDmg(guard.x,guard.y-guard.size-12,'FROSTGLASS SURGE','#bff4ff',{sz:11,bold:true,outline:'#061433'});
    shake(4);
    return;
  }
  ...
  const hits=[];
  for(const u of players){
    if(!(dragonIsTank(u)||dragonIsMelee(u)))continue;
    const dx=u.x-guard.x,dy=u.y-guard.y,d=Math.hypot(dx,dy);
    if(d>range+(u.size||20)*0.35)continue;
  ...
  addDmg(guard.x,guard.y-guard.size-12,'MIRRORICE CRUSH','#eef8ff',{sz:11,bold:true,outline:'#061433'});
  shake(5);
}
function updateDragonSafeZone(b,ctx){
  if(!b._dragonSkyPhase)return;
  const guards=allDragonSkyGuards(b,ctx.enemies);
  const alive=guards.filter(g=>g.hp>0);
  const killed=Math.max(0,(b._dragonGuardSpawned||guards.length)-alive.length);
  if(killed>=1&&!b._dragonSafeZone){
    b._dragonSafeZone=dragonSafeZonePoint(b,ctx);
    b._dragonSafeZoneActive=false;
    ctx.addDamageText(b._dragonSafeZone.x,b._dragonSafeZone.y-18,'SAFE ICE FORMING','#d8f8ff',{sz:12,bold:true,outline:'#061433'});
    ctx.groundFx.push({x:b._dragonSafeZone.x,y:b._dragonSafeZone.y,r:0,maxR:b.diamondSafeZoneRadius||96,life:0.9,color:'#d8f8ff',safeZone:true,active:false,label:'SAFE ICE'});
  }
  if(killed>=2&&b._dragonSafeZone&&!b._dragonSafeZoneActive){
    b._dragonSafeZoneActive=true;
    b._dragonSafeZoneActivatedFrame=ctx.frame||0;
    ctx.addDamageText(b._dragonSafeZone.x,b._dragonSafeZone.y-22,'SAFE ICE','#eef8ff',{sz:14,bold:true,outline:'#061433'});
    ctx.showFlash('SAFE ICE!  MOVE IN','#d8f8ff',58);
    ctx.shake(5);
  }
  if(b._dragonSafeZone&&(ctx.frame||0)%12===0){
    ctx.groundFx.push({x:b._dragonSafeZone.x,y:b._dragonSafeZone.y,r:0,maxR:b.diamondSafeZoneRadius||96,life:0.28,color:'#d8f8ff',safeZone:true,active:!!b._dragonSafeZoneActive,label:'SAFE ICE'});
  }
}
function resolveDragonJudgment(b,ctx){
  if(b._dragonJudgmentResolved)return;
  b._dragonJudgmentResolved=true;
  const players=dragonPlayers(ctx.units||[]);
  const zone=b._dragonSafeZone;
  const safeActive=!!(b._dragonSafeZoneActive&&zone);
  const radius=b.diamondSafeZoneRadius||96;
  const base=(b.diamondJudgmentDmg||250)*dragonSkillDamageMult(b);
  ctx.groundFx.push({x:b.x,y:b.y,r:0,maxR:Math.max(260,(b.size||70)*3.65),life:0.84,color:'#eef8ff',enemyWarn:true,warnTimer:24,warnMax:24,label:'JUDGMENT'});
  for(const u of players){
    const inside=safeActive&&Math.hypot((u.x||0)-zone.x,(u.y||0)-zone.y)<=radius;
    const safeMult=inside?(b.diamondJudgmentSafeMult||0.30):(b.diamondJudgmentOutsideMult||1);
    const roleMult=dragonIsTank(u)?0.95:(dragonIsMelee(u)?0.90:1);
    const dmg=Math.round(base*safeMult*roleMult);
    ctx.beamFx.push({x1:b.x,y1:b.y-(b.size||70)*0.45,x2:u.x,y2:u.y-(u.size||20)*0.20,life:24,maxLife:24,color:inside?'#d8f8ff':'#eef8ff',width:inside?3.4:5.2,straight:false});
    ctx.dealDamage(u,dmg,b,'magic','diamondJudgment',{sourceLabel:'DIAMOND JUDGMENT',sourceColor:'#eef8ff'});
    ctx.addDamageText(u.x,u.y-(u.size||20)-8,inside?'SAFE ICE':'DIAMOND JUDGMENT',inside?'#d8f8ff':'#eef8ff',{sz:11,bold:true,outline:'#061433'});
    ctx.addParticle(u.x,u.y,'#d8f8ff',inside?7:12,3);
  }
  ctx.addDamageText(b.x,b.y-(b.size||70)-20,'DIAMOND JUDGMENT','#eef8ff',{sz:15,bold:true,outline:'#061433'});
  ctx.showFlash(safeActive?'DIAMOND JUDGMENT - SHELTERED':'DIAMOND JUDGMENT!','#eef8ff',72);
  ctx.shake(safeActive?8:13);
}
function startDragonDiamondStorm(b,ctx){
  ...
  anchorWinterglassDragon(b,ctx,true);
  groundFx.push({x:b.x,y:b.y,r:0,maxR:Math.max(250,(b.size||70)*3.45),life:0.9,color:'#eef8ff',enemyWarn:true,warnTimer:42,warnMax:42,label:'JUDGMENT'});
  addDmg(b.x,b.y-(b.size||70)-16,'DIAMOND JUDGMENT','#eef8ff',{sz:15,bold:true,outline:'#061433'});
  addDmg(b.x,b.y-(b.size||70)-34,'DRAGON IMMUNE','#d8f8ff',{sz:12,bold:true,outline:'#061433'});
  for(let i=0;i<46;i++)addP(b.x+rnd(-b.size*1.2,b.size*1.2),b.y+rnd(-b.size,b.size*0.7),'#d8f8ff',1,5);
  showFlash('DIAMOND JUDGMENT!','#eef8ff',75);
  b._dragonStormCasts=(b._dragonStormCasts||0)+1;
  spawnDragonJudgmentGuards(b,ctx);
  shake(10);
  return true;
}
function endDragonDiamondStorm(b,ctx){
  const { enemies, groundFx, addParticle:addP, addDamageText:addDmg, showFlash, shake }=ctx;
  if(!b._dragonJudgmentResolved)resolveDragonJudgment(b,ctx);
  const safeSuccess=!!b._dragonSafeZoneActive;
...
function tickDragonSkyPhase(b,ctx){
  ...
  b._dragonSkyTimer--;
  spawnDragonJudgmentGuards(b,ctx);
  for(const enemy of activeDragonSkyAdds(b,ctx.enemies)){
    tickDragonSkyGuard(enemy,b,ctx);
  }
  updateDragonSafeZone(b,ctx);
  ...
  if(b._dragonSkyTimer<=5*GAME_TICK_HZ&&b._dragonSkyTimer%GAME_TICK_HZ===0){
    addDmg(b.x,b.y-(b.size||70)-36,'JUDGMENT '+Math.max(1,Math.ceil(b._dragonSkyTimer/GAME_TICK_HZ))+'s','#eef8ff',{sz:12,bold:true,outline:'#061433'});
  }
  if(b._dragonSkyTimer<=6*GAME_TICK_HZ&&b._dragonSkyTimer%24===0){
    groundFx.push({x:b.x,y:b.y,r:0,maxR:Math.max(250,(b.size||70)*3.55),life:0.42,color:'#eef8ff',enemyWarn:true,warnTimer:18,warnMax:18,label:'JUDGMENT'});
  }
...
function updateWinterglassDragon(b,ctx){
  ...
  if(tryCast('_dragonMawCd',b.frigidMawCD||1200,58,castDragonFrigidMaw))return;
  if(tryCast('_dragonWallCd',b.iceWallCD||1560,54,castDragonIceWall))return;
