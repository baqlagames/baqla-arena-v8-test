...
export function applyUnitPassives(u,unitIdx,lv,{gameTickHz,signatures}){
  ...
  if(unitIdx===3&&!u.branch){
    ...
    u.holySwordSaintCombo={
      ...
      fivefoldGuardDr:0.10,fivefoldGuardDur:2*GAME_TICK_HZ,fivefoldDamageBuff:0.08,fivefoldDamageBuffDur:4*GAME_TICK_HZ,
      ...
      crownCrossGuardDr:0.12,crownCrossGuardDur:3*GAME_TICK_HZ,crownCrossDamageBuff:0.15,crownCrossDamageBuffDur:5*GAME_TICK_HZ,
  ...
  if(lv>=3){
    ...
    if(unitIdx===10&&!u.branch){ // Naana Holy - reactive raid healer identity
      u.holyRenew={cd:0,every:Math.round(3*GAME_TICK_HZ),dur:Math.round(6*GAME_TICK_HZ),healPct:0.025,count:2};
      u.holyFlashHeal={every:3,counter:0,healPct:0.08};
      u.holySanctify={healPct:0.07,radius:150,count:4};
      u.holyComfortAura={cd:0,every:Math.round(2*GAME_TICK_HZ),healPct:0.01};
    }
  }
  ...
  if(arena_isCapstoneLevel(lv)){
    ...
    if(unitIdx===10&&u.branch!=='b'){ // Naana non-Shadow L5: Prayer bounces +2, Spirit of Redemption duration +3s
      ...
      if(!u.branch&&u.holyFlashHeal)u.holyFlashHeal.healPct=0.10;
...
export function applyPassiveToUnit(u,id,sc,boost,{gameTickHz}){
  ...
  switch(id){
    ...
    case 'prayerOfMending': u.prayerOfMending={cd:0,every:Math.round(6*GAME_TICK_HZ),maxBounces:5,healPct:0.18*boost}; break;
