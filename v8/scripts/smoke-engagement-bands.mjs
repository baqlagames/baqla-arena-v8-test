...
function moveMany(actor, tx, ty, ticks = 240) {
...

{
  ...
  assert(melee.y >= bands.playerMeleeY - 0.01, `melee crossed middle band (${melee.y} < ${bands.playerMeleeY})`);
  assert(Math.abs(melee.y - bands.playerMeleeY) < 8, `melee did not settle near middle band (${melee.y})`);
}

{
  ...
  assert(ranged.y >= bands.playerRangedY - 0.01, `ranged crossed firing band (${ranged.y} < ${bands.playerRangedY})`);
}

{
  ...
  assert(diver.y <= bands.playerMeleeY, 'diver should be allowed ahead of normal melee but not outside arena');
}

{
  const melee = { x: 250, y: 720, homeX: 250, homeY: 720, speed: 0.3, range: 44, size: 24, isPlayer: true, arch: 'melee' };
  const reachable = { x: 250, y: bands.playerMeleeY - 30, hp: 100, size: 24 };
  const tooDeep = { x: 250, y: bounds.arenaTop + 30, hp: 100, size: 24 };
  assert(isReachableFromLeash(melee, reachable, bounds), 'middle enemy should be reachable from melee band');
  assert(!isReachableFromLeash(melee, tooDeep, bounds), 'top enemy should not pull melee beyond band');
}

{
  ...
  assert(unit.y >= bands.playerMeleeY - 0.01, `no-target melee advance crossed middle band (${unit.y})`);
}
...
console.log('Engagement band smoke passed for melee/ranged advance, player dash clamps, and enemy dive bounds.');
